  ...
  def getAction(self, gameState):
    ...
    def max_value(gameState, treeDepth, agentIndex):
        ...
        for action in gameState.getLegalActions(agentIndex):
            val.append(value(gameState.generateSuccessor(agentIndex, action), treeDepth-1, (treeDepth+1) % numAgents))
        return max(val), gameState.getLegalActions(agentIndex)[val.index(max(val))]
    ...
    def min_value(gameState, treeDepth, agentIndex):
        ...
        numAgents = gameState.getNumAgents()
        for action in gameState.getLegalActions(agentIndex):
            val.append(value(gameState.generateSuccessor(agentIndex, action), treeDepth-1, (treeDepth+1) % number_of_agents))
        return max(val), gameState.getLegalActions(agentIndex)[val.index(max(val))]
    ...
    def value(gameState, treeDepth, agentIndex):
        ...
        if agentIndex == 0:
            return max_value(gameState, treeDepth, agentIndex)
        else: return min_value(gameState, treeDepth, agentIndex)

    numAgents = gameState.getNumAgents()
    val, action = value(gameState, self.depth * numAgents, 0)
    return action
