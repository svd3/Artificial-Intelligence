...
class CSP:
    def __init__(self):
        ...
        #      binaryPotentials[0][1][0][1] == f1('b','b') * f2('b','b')
        #      binaryPotentials[0][1][1][0] == f1('c','a') * f2('c','a')
        #      binaryPotentials[0][1][1][1] == f1('c','b') * f2('c','b')
...
# - self.semesters: semesters without the years (e.g., Fall)
...
class Course:
    ...
    # Return whether this course is offered in |semester| (e.g., Fall2013).
    def is_offered_in(self, semester):
        return any(semester.startswith(q) for q in self.semesters)

    def short_str(self): return '%s: %s' % (self.cid, self.name)

    def __str__(self):
        return 'Course{cid: %s, name: %s, semesters: %s, units: %s-%s, prereqs: %s}' % (self.cid, self.name, self.semesters, self.minUnits, self.maxUnits, self.prereqs)
...
class Request:
    def __init__(self, cids, semesters, prereqs, weight):
        """
    ...
    def __str__(self):
        return 'Request{%s %s %s %s}' % \
            (self.cids, self.semesters, self.prereqs, self.weight)
...
class Profile:
    def __init__(self, bulletin, prefsPath):
        ...
        for line in open(prefsPath):
            ...
            # Register a semester (semester, year)
            m = re.match('register (.+)', line)
            if m:
                semester = m.group(1)
                m = re.match('(Fall|Spr)(\d\d\d\d)', semester)
                if not m:
                    raise Exception("Invalid semester '%s', want something like Spr2013" % semester)
            ...
            if m:
                ...
                for i in range(0, len(args), 2):
                    ...
                    elif args[i] == 'in':  # Take in a particular semester
                        semesters = [self.ensure_semester(q) for q in args[i+1].split(',')]
                ...
                self.requests.append(Request(cids, semesters, prereqs, weight))
                continue
    ...
    def print_info(self):
        ...
        print "Semester: %s" % self.semesters
    ...
    def ensure_semester(self, semester):
        if semester not in self.semesters:
            raise Exception("Invalid semester: '%s'" % semester)
        return semester
...
def extract_course_scheduling_solution(profile, assign):
    ...
    for req in profile.requests:
        for cid in req.cids:
            ...
            if semester == None:
                continue
            if (cid, semester) not in assign:
                results.append((semester, cid, None))
            else:
                results.append((semester, cid, assign[(cid, semester)]))
    return results
